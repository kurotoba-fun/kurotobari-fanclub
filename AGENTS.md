...
| `elliott` | エリオット | 金髪 | 青 | ルキと配色が重なる。右頬に火傷痕がなく、投稿文などにエリオットと判断できる根拠がある場合に分類する |
...
| `tobarimori` | 帳守 | 赤紫の逆立った短髪（素顔差分） | 鮮やかなピンク（素顔差分） | ガスマスクを最優先。素顔差分では鋭い歯、黄色い上着、鋲付きの黄色いヘッドバンドも強い指標 |
...
| `junze` | 俊哲 | 濃紺にも見える黒 | 黄緑〜金系（開眼時） | 細く閉じた糸目、目尻の赤い隈取、漢服、長い房飾り、胡散臭い笑顔 |
| `pei` | 霈 | 濃紺の短髪 | 青緑 | 口元の黒子、長い睫毛、アンニュイな顔立ち、シックな服装 |
| `natsume` | 棗 | 白金のマッシュ | 資料未確定 | 重い前髪のメカクレ、多数のピアスとタトゥー、パンク・ストリート系 |
| `luki` | ルキ | 金の短髪 | 碧眼 | 三白眼、右頬の大きな火傷痕、中性的な顔立ち |
| `hayase` | 湍（重三 湍） | 赤 | 紫 | 短髪、片側スリックバック、左眉上の傷、赤いライダースジャケット |
...
- **ルキとエリオット**: 両者とも金髪・青系の瞳のため、配色だけでは区別できない。右頬の広範囲な火傷痕が見えればルキと判定する。火傷痕が構図や角度で見えない場合も、ギャラリーではルキの出現数が圧倒的に多いことを事前確率として扱い、明確なエリオットの根拠がなければルキを優先する。
- **調と白瀬**: 橙色の瞳または大型ヘッドホンが確認できれば調を優先する。白瀬は藍〜青の瞳と茶色の癖毛を組み合わせて判断する。髪色だけでは区別しない。
- **霈と灼**: 霈は濃紺の整った短髪、青緑の瞳、口元の黒子、長い睫毛、細身の体格を組み合わせて判断する。灼は無造作な黒髪、切れ長の緑の瞳、犬歯、筋肉質な体格が特徴。特に口元の黒子と細身の体格があれば霈を優先する。
- **俊哲と王 逸翔**: 中華風の服だけで王 逸翔に分類しない。細く閉じた糸目、目尻の赤い隈取、長い房飾りがあれば俊哲を優先する。王 逸翔は黒髪のオールバック、赤い吊り目、扇子を主な指標とする。
- **帳守と湍**: 赤系の髪だけで湍に分類しない。帳守の素顔差分は鮮やかな赤紫の逆立った髪、ピンクの瞳、鋭い歯、黄色い上着、鋲付きヘッドバンドが特徴。湍は片側スリックバック、紫の瞳、左眉上の傷、赤いライダースジャケットで区別する。
...
`localhost`、`127.0.0.1`、`::1` でギャラリーを開いた時だけ、公開サイトには出ない管理UIが表示されます。

- 各カードのチェックボックスで複数IDを選び、「選択したIDをコピー」で一覧をコピーできる。
- 各カードの「IDコピー」で、その画像のIDだけをコピーできる。
- 「IDコピー」を右側のキャラクターボックスへドラッグすると、振り分け修正候補として登録される。
- 複数の画像を振り分けた後に「修正依頼文をコピー」を押すと、キャラクター名ごとにIDをまとめた依頼文をコピーできる。
- 同じIDを別のボックスへドロップすると、最後に指定したキャラクターへ移動する。
- `掲載対象外` ボックスも同じ方法で利用できる。
